...
def test_batch_task_counts_endpoint(client, mock_supabase_client):
    ...
    # Force Supabase mode by mocking both is_asyncpg_mode and get_database_mode
    with patch("src.server.services.projects.task_service.is_asyncpg_mode", return_value=False):
        with patch("src.server.services.projects.task_service.get_database_mode", return_value="supabase"):
            with patch("src.server.services.client_manager.get_database_mode", return_value="supabase"):
